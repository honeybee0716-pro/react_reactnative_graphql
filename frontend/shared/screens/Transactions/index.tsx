...
export default function Transaction(props: any) {
  ...
  const handleS = async ()=>{
    cT({
      ...
      onCompleted: async (createTransactionData) => {
        ...
        if (
          createTransactionData?.createTransaction?.status === 'success'
        ) {
          toast.show({
  ...
  return (
    <>
      ...
      return <Button {...triggerProps}>create a new transaction</Button>;
    }}>
        <Popover.Content width="56">
          ...
          <Popover.Header>Transaction Details</Popover.Header>
          <Popover.Body>
            <FormControl mt="3">
              ...
              <input type="number"  onChange={e => setItem0({ ...item0, purchaseAmount: e.target.value })} />
            </FormControl>
          </Popover.Body>
          <Popover.Footer>
            <Button.Group>

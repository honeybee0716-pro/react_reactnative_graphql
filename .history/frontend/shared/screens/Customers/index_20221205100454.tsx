...
export default function Customer(props: any) {
  ...
  const [getC] = useLazyQuery(SEARCH_FOR_LEADS)
  ...
  const getCs = () => {
    getC({
      onCompleted: async ({ searchForLeads }) => {
        if (searchForLeads?.status === 'success') {
          console.log(searchForLeads.dataBusiness)
          setCdata(searchForLeads.dataBusiness)
          setFdata(searchForLeads.dataBusiness)
          setTableLoading(false)
        }
        if (searchForLeads?.message) {
          toast.show({
            description: searchForLeads.message
          })
          return
        } else {
          toast.show({
